-- U18e — dedupe public crawl telemetry (middleware double-fire / prefetch).

alter table public.public_crawl_events
  add column if not exists resource_key text;

create index if not exists public_crawl_events_dedupe_idx
  on public.public_crawl_events (
    route,
    agent_bucket,
    is_bot,
    recorded_at desc
...
drop function if exists public.record_public_crawl_event(text, text, uuid, text, text, text, text, text);
...
revoke all on function public.record_public_crawl_event(text, text, uuid, text, text, text, text, text, text) from public;
grant execute on function public.record_public_crawl_event(text, text, uuid, text, text, text, text, text, text) to anon, authenticated;
