-- Add living/private flags to persons and tighten visibility policies
...
as $$
  select public.can_read_tree(tree_id)
    and (
      not coalesce(person_is_private, false)
      or public.can_write_tree(tree_id)
    );
$$;
...
create policy "persons_select" on public.persons for select using (public.person_visibility(tree_id, is_private));
  ...
  using (
    exists (
      ...
      where p.id = person_id
        and public.person_visibility(p.tree_id, p.is_private)
    )
  );
...
create policy "relationships_select" on public.relationships
  for select
  using (
    public.can_read_tree(tree_id)
    and (
      public.can_write_tree(tree_id)
      or (
        exists (
...
drop policy if exists "media_person_link_select" on public.media_person_links;
create policy "media_person_link_select" on public.media_person_links
  for select
  using (
    exists (
      select 1 from public.persons p
...
drop policy if exists "dna_tests_select" on public.dna_tests;
create policy "dna_tests_select" on public.dna_tests
  for select
  using (
    exists (
      select 1 from public.persons p
...
drop policy if exists "dna_matches_select" on public.dna_matches;
