-- U18g — tag signed-in viewer traffic and exclude current user in admin stats.

alter table public.public_crawl_events
  add column if not exists viewer_user_id uuid;

create index if not exists public_crawl_events_viewer_user_idx
  on public.public_crawl_events (viewer_user_id, recorded_at desc)
  where viewer_user_id is not null;

drop function if exists public.record_public_crawl_event(text, text, uuid, text, text, text, text, text, text);
...
revoke all on function public.record_public_crawl_event(text, text, uuid, text, text, text, text, text, text, uuid) from public;
grant execute on function public.record_public_crawl_event(text, text, uuid, text, text, text, text, text, text, uuid) to anon, authenticated;
