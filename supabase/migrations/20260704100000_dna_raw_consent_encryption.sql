-- K7: raw autosomal consent + encrypted storage metadata; K4 haplogroup metadata fields in upsert.

alter table public.dna_tests
  add column if not exists consent_given_at timestamptz,
  add column if not exists consent_scope text;

create or replace function public.admin_purge_dna_raw_data(
  target_test_id uuid,
...
as $$
declare
  target_record public.dna_tests;
  target_tree_id uuid;
...
begin
  ...
  join public.persons p on p.id = dt.person_id
  where dt.id = target_test_id
  for update of dt;

  if not found then
    raise exception 'DNA test % not found', target_test_id;
  end if;

  target_tree_id := (
    select p.tree_id from public.persons p where p.id = target_record.person_id
  );
  perform public.assert_can_write_tree(target_tree_id);

  cleaned_metadata := coalesce(target_record.metadata, '{}'::jsonb)
    - 'encryptedRawPayload'
    - 'rawMarkerIndexStats'
    - 'rawDataPreview';

  update public.dna_tests
  set
  ...
  return jsonb_build_object('test_id', target_test_id, 'purged', true);
end;
$$;

revoke all on function public.admin_purge_dna_raw_data(uuid, uuid, text) from public, anon;
grant execute on function public.admin_purge_dna_raw_data(uuid, uuid, text) to authenticated;

-- Extend admin_upsert_person_dna_tests metadata merge + consent columns.
...
declare
  ...
  parsed_consent_scope text;
  parsed_is_private boolean;
...
begin
  ...
  perform public.assert_can_write_tree(target_record.tree_id);
  ...
  loop
    ...
    if coalesce(dna_item->>'consentGivenAt', '') ~ '^\d{4}-' then
      parsed_consent_at := (dna_item->>'consentGivenAt')::timestamptz;
    end if;
    parsed_consent_scope := nullif(dna_item->>'consentScope', '');

    parsed_is_private := coalesce((dna_item->>'isPrivate')::boolean, false)
      or parsed_consent_scope = 'raw_autosomal_storage'
      or coalesce(dna_item->'metadata', '{}'::jsonb) ? 'encryptedRawPayload';

    merged_metadata := jsonb_strip_nulls(
      ...
      || jsonb_build_object(
        ...
        'yHaplogroup', nullif(dna_item->>'yHaplogroup', ''),
        'mtDnaHaplogroup', nullif(dna_item->>'mtDnaHaplogroup', ''),
        'mitotree', nullif(dna_item->>'mitotree', ''),
        'rawDataSummary', coalesce(dna_item->'rawDataSummary', 'null'::jsonb),
        'rawDataPreview', case
          when coalesce(dna_item->'metadata', '{}'::jsonb) ? 'encryptedRawPayload' then 'null'::jsonb
          else coalesce(dna_item->'rawDataPreview', 'null'::jsonb)
        end,
        'encryptedRawPayload', coalesce(dna_item->'metadata'->'encryptedRawPayload', 'null'::jsonb),
        'rawMarkerIndexStats', coalesce(dna_item->'metadata'->'rawMarkerIndexStats', 'null'::jsonb),
