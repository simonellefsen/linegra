-- Load other parents of the focus person's children (coparent inference).
...
begin
  ...
  spouse_ids as (
    select case
      when d.person_id = target_person_id then d.related_id
      else d.person_id
    end as spouse_person_id
    from direct d
    where d.type in ('marriage', 'partner')
  ),
  spouse_child_rels as (
    select
      r.id,
      r.tree_id,
      r.person_id,
      ...
      r.created_at
    from public.relationships r
    inner join spouse_ids s on s.spouse_person_id = r.person_id
    where r.tree_id = target_tree_id
      and r.type::text = any(parental_types)
  ),
  focus_child_ids as (
    select distinct d.related_id as child_id
    from direct d
    where d.person_id = target_person_id
      and d.type::text = any(parental_types)
  ),
  coparent_rels as (
    select
      r.id,
      ...
      r.created_at
    from public.relationships r
    inner join focus_child_ids fc on fc.child_id = r.related_id
    where r.tree_id = target_tree_id
      and r.type::text = any(parental_types)
  ...
  all_rels as (
    ...
    union
    select * from spouse_child_rels
    union
    select * from coparent_rels
  ),
  visible_rels as (
    select ar.*
    from all_rels ar
