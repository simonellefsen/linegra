-- Keep gender changes safe when earlier imports created duplicate generic parent links.
-- A generic `child` link is one parent-to-child assertion, so retain its earliest row only.
with ranked_child_links as (
  select
    id,
    row_number() over (
      partition by tree_id, person_id, related_id
      order by created_at asc, id asc
    ) as row_number
  from public.relationships
  where type = 'child'
)
delete from public.relationships relationship_row
using ranked_child_links ranked
where relationship_row.id = ranked.id
  and ranked.row_number > 1;

create unique index if not exists relationships_unique_child
  on public.relationships(tree_id, person_id, related_id)
  where type = 'child';

-- Preserve the existing clearable-field behavior while replacing the profile RPC below.

create or replace function public.profile_payload_text(
  profile jsonb,
  snake_key text,
  camel_key text,
  current_value text,
  empty_to_null boolean default true
)
returns text
language sql
immutable
as $$
  select case
    when profile ? snake_key or profile ? camel_key then
      case
        when empty_to_null then
          nullif(btrim(coalesce(profile->>snake_key, profile->>camel_key, '')), '')
        else
          btrim(coalesce(profile->>snake_key, profile->>camel_key, ''))
      end
    else
      current_value
  end;
...
begin
  ...
  v_tree_id := target_record.tree_id;
  perform public.assert_can_write_tree(v_tree_id);
  ...
  set
    first_name = public.profile_payload_text(payload_profile, 'first_name', 'firstName', first_name, false),
    last_name = public.profile_payload_text(payload_profile, 'last_name', 'lastName', last_name, false),
    maiden_name = public.profile_payload_text(payload_profile, 'maiden_name', 'maidenName', maiden_name),
    gender = next_gender::gender_type,
    birth_date_text = public.profile_payload_text(payload_profile, 'birth_date_text', 'birthDate', birth_date_text),
    birth_place_text = public.profile_payload_text(payload_profile, 'birth_place_text', 'birthPlaceText', birth_place_text),
    death_date_text = public.profile_payload_text(payload_profile, 'death_date_text', 'deathDate', death_date_text),
    death_place_text = public.profile_payload_text(payload_profile, 'death_place_text', 'deathPlaceText', death_place_text),
    residence_at_death_text = public.profile_payload_text(payload_profile, 'residence_at_death_text', 'residenceAtDeathText', residence_at_death_text),
    burial_date_text = public.profile_payload_text(payload_profile, 'burial_date_text', 'burialDate', burial_date_text),
    burial_place_text = public.profile_payload_text(payload_profile, 'burial_place_text', 'burialPlaceText', burial_place_text),
    death_cause = public.profile_payload_text(payload_profile, 'death_cause', 'deathCause', death_cause),
    death_cause_category = public.profile_payload_text(payload_profile, 'death_cause_category', 'deathCauseCategory', death_cause_category),
  ...
  if next_gender = 'M' then
    ...
    where r.tree_id = v_tree_id
      ...
      and exists (
        ...
        where existing.tree_id = v_tree_id
          and existing.related_id = r.related_id
          and existing.type = 'bio_father'
      );
...
revoke all on function public.profile_payload_text(jsonb, text, text, text, boolean) from public;
grant execute on function public.profile_payload_text(jsonb, text, text, text, boolean) to authenticated, service_role;
