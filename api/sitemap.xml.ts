...
export default async function handler(request: Request): Promise<Response> {
  ...
  const { data, error } = await supabase.rpc('list_public_sitemap_entries', { entry_limit: 10000 });
  if (error) {
  ...
  const urls = rows
    .map((row) => {
      let loc = origin;
      if (row.kind === 'tree' && row.tree_id) loc = buildTreeUrl(row.tree_id, origin);
      if (row.kind === 'person' && row.tree_id && row.person_id) {
        loc = buildPersonUrl(row.tree_id, row.person_id, origin);
      }
