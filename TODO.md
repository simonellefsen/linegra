...
- [ ] **Y1 — Split `services/archive.ts`** by domain behind a barrel export; move pure mappers to
      `lib/` with tests. *(2026-07-05 slice: `lib/archiveDbMappers.ts` + tests; `services/archive/`
      trees, collaborators, aiUsage, publicResolve; `archive.ts` ~3.8k lines, DNA/persons/gedcom remain.)*
